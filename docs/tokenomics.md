...
Also, we are pretty sure that it is impossible to create a single-coin tokenomics model in which transaction growth results in gains for all participants, so Antofy tokenomics uses two coins: RAY and ABN. Their supply is managed by Antofy Grinder, which is essentially an automated central bank.
...
RAY can only be released into circulation by burning ABN in the Antofy Grinder. There is a queue of orders arranged by the ABN / RAY rate for this purpose. The more ABNs are offered in exchange for a single RAY, the sooner the Grinder will process that order. The closest analogy to such a queue is exchange limit sell orders, executed by persistent demand from the Grinder.
...
The Antofy Grinder controls the issuance of the RAY coin to reach the target circulating supply determined by a mathematical formula. Since the RAY coin is inflationary and the commissions paid in RAY are burned, there is a constant need to create new RAYs. Antofy Grinder will print RAYs providing the necessary issuance and replacing burned RAY coins to reach the target RAY circulating supply.
...
<table><thead><tr><th width="177">Parameter</th><th>Value</th></tr></thead><tbody><tr><td>Maximum Supply</td><td>100 000 000 ABNs</td></tr><tr><td>Utility</td><td>ABN is used to issue RAY</td></tr><tr><td>Issuance</td><td>Can be minted only as validator's reward</td></tr><tr><td>Burning</td><td>A part of ABN while minting RAY is burned </td></tr></tbody></table>
...
In Antofy, ABN is designed for investment purposes and is directly secured by user activity. ABN is a RAY coin printing certificate. Its main properties are a constant demand from the system and persistent burning. Thus, the more RAY coins are burned as transaction fees, the more must be printed, and the more ABNs will be used for this purpose, part of which will be burned.
...
ABN is used by Grinder for RAY issuance. Anyone can put ABN coins in the Grinder specifying ABN / RAY rate. The Grinder continuously uses the ABNs offers with the highest bids to issue RAYs. During the order processing, one part of the ABN burns irretrievably, and the other part becomes available for reissue into circulation. It will stop being burned when only 1 000 000 ABNs remain.
...
{% tab title="Details" %}
The percantage of ABN burned depends on the amount of ABNs put in Grinder for RAY issuance. The formula is:

$$
ABN_{\%burned}\;=\;100\cdot\frac{ABN_{grindered}}{ABN_{grindered}+10^7}
$$

When the number of ABNs used for RAY issuance exeeds 990 000 000 there will remain only 1 000 000 of ABN coins and they will stop burning.
{% endtab %}
{% endtabs %}

#### **ABN Issuance**

Validators' rewards will be calculated in RAY as the sum of burned commissions and inflationary RAY issuance. ABN will be issued corresponding to that amount using a constant product formula as validators' rewards. This means that the reduction of ABN token issuance will not occur in leaps and bounds, as in the case of Bitcoin halvings, but gradually.
...
{% tab title="Details" %}
The ABN supply depends on the number of RAYs minted by the Grinder. That includes all newly minted RAYs (issuance and replacement for burned coins). The number of ABNs to mint is defined by the formula:

$$
ABN_{issued}=\frac{10^{16}}{RAY_{minted}+10^8}
$$

The Grinder uses a virtual ABN system pool to issue ABN. The Grinder can only take ABN from there to the extent of the burned commissions and inflation issuance. ABN used but not burned during RAY minting returns to this virtual pool. Thus, over time, fewer and fewer ABN will be issued as a reward.

You can imagine - that there is a virtual AMM in Grinder with a pool of **RAY totally minted** / **ABN for issuance**. But the calculation of the ABN issue is done without having a real AMM - just by the constant product formula.
{% endtab %}
{% endtabs %}

#### ABN Circulating Supply

The amount of ABN circulating supply depends on the ratio of the ABN burning speed to the issuance volume. The ABN burning pace is determined primarily by the ABN market price. The ABN issuance depends on the number of RAYs burned as commissions. Thus, the exact amount of ABN in circulation can only be approximated.
...
It assumes that the number of transactions grows from 0 to a given number within about half a year, and the rate at which Grinder uses ABN is directly proportional to the ratio of RAY and ABN coins in circulation.
{% endtab %}
{% endtabs %}

#### RAY / ABN Rate

The RAY / ABN rate depends very much on the initial distribution of coins - therefore, it can even fall during the first few years if the number of transactions is not very high. But in the long term, the amount of RAY in circulation will grow, and the amount of ABN will fall after the peak is passed. So, in the long run, depending on the number of transactions in the blockchain, the amount of RAY given per ABN will steadily grow.
...
{% tab title="Details" %}
The RAY / ABN rate is calculated based on a computer model as a ratio of the RAY circulating supply and the ABN circulating supply.&#x20;

The model assumes that the number of transactions grows from 0 to a given number within about half a year, and the rate at which Grinder uses ABN is directly proportional to the ratio of RAY and ABN coins in circulation.
{% endtab %}
{% endtabs %}
...
Bridges to other popular blockchains will be created in the future. If necessary, it will be possible to make a bridge with reverse logic, where RAY or ABN is withdrawn from Elysium, and in another blockchain, the user receives wrapped RAY or ABN tokens.

### What happens if no one puts ABN in the Grinder?

That won't happen. At least, the Elysium Team will put a single ABN in the Grinder queue at a price sufficient to issue a billion RAYs. And we have no doubt that there will be many such overpriced orders.

### ABN will not be burned when trading on the exchanges. Will it break the system?

There will be DEX in Elysium, so it will be possible to trade ABN for RAY outside the Grinder.&#x20;
...
As has already been said, the prices in the Grinder queue and on the exchange correlate. If the price of ABN falls (relative to RAY), then the Grinder will start to consume (and burn) significantly more ABN. Therefore, at moments of deep ABN drawdowns, it will be burned at an accelerated pace.
